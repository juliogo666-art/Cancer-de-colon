...
# def generate_gradcam_pytorch(model, img_pil, target_layer):

#     """
#     Genera el mapa de calor Grad-CAM para modelos PyTorch.
#     """
#     model.eval()
...
#     # Variables para capturar gradientes y activaciones
#     gradients = []
#     activations = []
...
#     # Backward pass (usamos la salida directa ya que es binaria)
#     output.backward()
...
#     # Crear imagen final (superposición)
...
# def generate_gradcam_pytorch(model, img_pil, target_layer):
#     """
#     Genera Grad-CAM para un modelo PyTorch y lo superpone a la imagen.
#     """
#     model.eval()
    
#     # 1. Preprocesar (idéntico al entrenamiento)
#     transform = transforms.Compose([
#         transforms.Resize((224, 224)),
#         transforms.ToTensor(),
#         transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
...
#     # 2. Hooks para capturar gradientes y activaciones
#     gradients = []
#     activations = []
...
#     # 3. Forward y Backward (Sin no_grad para tener gradientes)
#     model.zero_grad()
#     output = model(input_tensor)
#     output.backward()

#     # 4. Procesar el mapa de calor
#     grad = gradients[0].detach()
#     act = activations[0].detach()
#     weights = torch.mean(grad, dim=(2, 3), keepdim=True)
#     cam = torch.sum(weights * act, dim=1).squeeze().cpu().numpy()
...
#     # 5. Post-procesado del mapa
#     cam = np.maximum(cam, 0)
#     cam = cv2.resize(cam, (224, 224))
#     cam = (cam - cam.min()) / (cam.max() - cam.min() + 1e-8)

#     # 6. Superponer con la imagen original
#     img_cv = cv2.cvtColor(np.array(img_pil.resize((224, 224))), cv2.COLOR_RGB2BGR)
#     heatmap = cv2.applyColorMap(np.uint8(255 * cam), cv2.COLORMAP_JET)
#     result = cv2.addWeighted(img_cv, 0.6, heatmap, 0.4, 0)
...
def generate_gradcam_pytorch(model, img_pil, target_layer):
    """
    Versión robusta de Grad-CAM para PyTorch.
    """
    model.eval()
    ...
    # 2. Variables para capturar datos
    # Usamos un diccionario para evitar el error 'list index out of range'
    storage = {"grads": None, "acts": None}

    def backward_hook(module, grad_input, grad_output):
        storage["grads"] = grad_output[0].detach()
    def forward_hook(module, input, output):
        storage["acts"] = output.detach()

    # Registrar hooks
    h1 = target_layer.register_forward_hook(forward_hook)
    h2 = target_layer.register_full_backward_hook(backward_hook)

    # 3. Ejecutar pasada (con gradientes activados temporalmente)
    model.zero_grad()
    # Aseguramos que el input requiera gradiente para que el backward funcione
    input_tensor.requires_grad = True
    output = model(input_tensor)
    
    # IMPORTANTE: Si es clasificación binaria, hacemos backward del valor escalar
    ...
    # 4. Verificar si capturamos algo
    if storage["grads"] is None or storage["acts"] is None:
        raise ValueError("No se pudieron capturar gradientes. La capa objetivo podría ser inválida.")

    # 5. Calcular Grad-CAM
    weights = torch.mean(storage["grads"], dim=(2, 3), keepdim=True)
    cam = torch.sum(weights * storage["acts"], dim=1).squeeze().cpu().numpy()
