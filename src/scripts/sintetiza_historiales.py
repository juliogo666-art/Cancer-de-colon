...
#   python -m src.scripts.sintetiza_historiales
########################################################################################
...
def generar_datos_sinteticos(output_dir=None):
    """
    Genera un dataset sintético de 5000 pacientes simulados con factores de riesgo
    de cáncer de colon, pruebas médicas y diagnóstico.

    Parameters
    ----------
    output_dir : str, optional
        Directorio donde guardar el CSV. Si es None, usa la ruta por defecto
        relativa al archivo.
    ...
    dieta_rica_en_grasas = np.random.binomial(n=1, p=prob_dieta)
    ...
    # -------------------------------------------------------------------------
    # HISTORIAL MÉDICO: FACTORES HEREDITARIOS E INFLAMATORIOS
    # (Basado en el PDF asociacion ESP contra el cancer)
    # -------------------------------------------------------------------------
    ...
    fobt_result_str = ["Positive" if res == 1 else "Negative" for res in fobt_result]
    ...
    df_simulador = pd.DataFrame(
        {
            ...
            "Dieta_rica_en_grasas_animales": dieta_rica_en_grasas,
            ...
            "FOBT_Resultado (Sangre en heces)": fobt_result_str,
            "CEA_Level_ng_mL (Marcador Tumoral)": cea_level,
            "Diagnostico": diagnostico,
        }
    )

    # Guardar en CSV si se proporciona directorio
    ...
    output_path = os.path.join(output_dir, "pacientes_simulador_colon.csv")
    df_simulador.to_csv(output_path, index=False)
...
def sintetizar_historiales(df_global, output_dir):
    """
    Enriquece un DataFrame global con columnas sintéticas de factores de riesgo,
    pruebas médicas y diagnóstico basados en probabilidades clínicas.

    Parameters
    ----------
    df_global : pd.DataFrame
        DataFrame con datos globales de pacientes.
    output_dir : str
        Directorio donde guardar el CSV combinado.

    Returns
    -------
    pd.DataFrame
        DataFrame enriquecido con las nuevas columnas.
    """
    ...
    if "Target_Severity_Score" in df_global.columns:
        ...
        # Si no hay score, asumimos una probabilidad base
        df_global["diagnostico"] = np.random.binomial(1, 0.5, size=len(df_global))

    n = len(df_global)

    # Sintetizar datos para las nuevas columnas
    df_global["Dieta_rica_en_grasas_animales"] = np.random.binomial(n=1, p=0.3, size=n)

    # Si está enfermo, 65% de probabilidad de ser sedentario. Si está sano, 40%.
    prob_sedentarismo = np.where(df_global["diagnostico"] == 1, 0.65, 0.40)
    df_global["Sedentarismo"] = np.random.binomial(n=1, p=prob_sedentarismo, size=n)

    # La diabetes aumenta el riesgo, pero no es tan común.
    # Si está enfermo, 25% de probabilidad de tener diabetes. Si está sano, 10%.
    prob_diabetes = np.where(df_global["diagnostico"] == 1, 0.25, 0.10)
    df_global["Diabetes_tipo_2"] = np.random.binomial(n=1, p=prob_diabetes, size=n)

    prob_familiar = np.where(df_global["diagnostico"] == 1, 0.25, 0.10)
    df_global["Antecedentes_Familiares"] = np.random.binomial(
        n=1, p=prob_familiar, size=n
    )

    # 2. Componente hereditario (< 10% de todos los casos)
    # Le daremos un 8% a los enfermos y un 2% a los sanos.
    prob_hereditario = np.where(df_global["diagnostico"] == 1, 0.08, 0.02)
    df_global["Componente_Hereditario"] = np.random.binomial(
        n=1, p=prob_hereditario, size=n
    )

    # 3. Síndromes predisponentes (< 5% de todos los casos)
    # Le daremos un 4% a los enfermos y un 0.5% a los sanos.
    prob_sindromes = np.where(df_global["diagnostico"] == 1, 0.04, 0.005)
    df_global["Sindromes_Predisponentes"] = np.random.binomial(
        n=1, p=prob_sindromes, size=n
    )
    ...
    df_global["Enfermedad_Inflamatoria_Intestinal"] = np.random.binomial(
        n=1, p=prob_eii, size=n
    )
    ...
    df_global["FOBT_Resultado_n"] = np.random.binomial(n=1, p=fobt_prob, size=n)
    df_global["FOBT_Resultado (Sangre en heces)"] = df_global["FOBT_Resultado_n"].map(
...
def sintetizar_datos_kaggle(df_k, output_dir):
    """
    Enriquece el dataset de Kaggle con columnas sintéticas de factores de riesgo
    y pruebas médicas.

    Parameters
    ----------
    df_k : pd.DataFrame
        DataFrame con datos del dataset de Kaggle (ya limpiado y numerizado).
    output_dir : str
        Directorio donde guardar el CSV final.
    ...
    df_k["FOBT_Resultado (Sangre en heces)"] = df_k["FOBT_Resultado_n"].map(
        {1: "Positive", 0: "Negative"}
    )
    ...
    df_k["CEA_Level_ng_mL (Marcador Tumoral)"] = np.where(
        condicion == 1, cea_enfermo, cea_sano
    )
    df_k["CEA_Level_ng_mL (Marcador Tumoral)"] = (
        df_k["CEA_Level_ng_mL (Marcador Tumoral)"].clip(0.1, 50.0).round(2)
    )
...
# Solo se ejecuta si se llama directamente: python -m src.scripts.sintetiza_historiales
if __name__ == "__main__":
    print("Generando datos sintéticos de pacientes...")
