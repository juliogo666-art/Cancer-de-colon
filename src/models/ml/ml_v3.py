...
xgb_model = XGBClassifier(n_estimators=300, learning_rate=0.02, objective='multi:softprob', num_class=3, random_state=42)
xgb_model.fit(X_train, y_train)

print("⚡ Entrenando LightGBM Clínico...")
lgbm_model = LGBMClassifier(n_estimators=300, learning_rate=0.02, verbosity=-1, random_state=42)
lgbm_model.fit(X_train, y_train)

# --- 4. FUNCIÓN DE EVALUACIÓN ---
def evaluar_sistema(y_real, y_pred, nombre):
    acc = accuracy_score(y_real, y_pred)
    rec_high = recall_score(y_real, y_pred, labels=[2], average='macro') # Recall específico de clase High
